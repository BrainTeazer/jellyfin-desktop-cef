...
static void desc_ready(void* d, struct wp_image_description_v1*, uint32_t) {
    ((ImageDescContext*)d)->ready = true;
}
static void desc_ready2(void* d, struct wp_image_description_v1*, uint32_t, uint32_t) {
    ((ImageDescContext*)d)->ready = true;
}
...
void WaylandSubsurface::registryGlobal(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version) {
    ...
    if (strcmp(interface, wl_compositor_interface.name) == 0) {
        self->wl_compositor_ = static_cast<wl_compositor*>(wl_registry_bind(registry, name, &wl_compositor_interface, 4));
    } else if (strcmp(interface, wl_subcompositor_interface.name) == 0) {
        self->wl_subcompositor_ = static_cast<wl_subcompositor*>(wl_registry_bind(registry, name, &wl_subcompositor_interface, 1));
        ...
        self->wl_output_ = static_cast<wl_output*>(wl_registry_bind(registry, name, &wl_output_interface, 1));
    } else if (strcmp(interface, wp_viewporter_interface.name) == 0) {
        self->viewporter_ = static_cast<wp_viewporter*>(wl_registry_bind(registry, name, &wp_viewporter_interface, 1));
    }
}
...
bool WaylandSubsurface::initWayland(SDL_Window* window) {
    ...
    wl_display_        = static_cast<wl_display*>(SDL_GetPointerProperty(props, SDL_PROP_WINDOW_WAYLAND_DISPLAY_POINTER, nullptr));
    wl_surface* parent = static_cast<wl_surface*>(SDL_GetPointerProperty(props, SDL_PROP_WINDOW_WAYLAND_SURFACE_POINTER, nullptr));
    if (!wl_display_ || !parent)
        return false;
...
bool WaylandSubsurface::init(
    SDL_Window* window, VkInstance, VkPhysicalDevice, VkDevice, uint32_t, const char* const*, int, const VkPhysicalDeviceFeatures2*) {
    ...
    if (vkCreateDevice(physical_device_, &di, nullptr, &device_) != VK_SUCCESS)
        return false;
    ...
    auto fn    = reinterpret_cast<PFN_vkCreateWaylandSurfaceKHR>(vkGetInstanceProcAddr(instance_, "vkCreateWaylandSurfaceKHR"));
    if (!fn || fn(instance_, &si, nullptr, &vk_surface_) != VK_SUCCESS)
        return false;
...
void WaylandSubsurface::queryDisplayProfile() {
    ...
    if (ic.max_lum > 0 && ic.ref_lum > 0) {
        ...
        LOG_INFO(LOG_PLATFORM,
                 ...
                 display_profile_.max_luma,
                 display_profile_.min_luma,
                 display_profile_.ref_luma);
    }
...
int WaylandSubsurface::deviceExtensionCount() const {
    return (int)enabled_extensions_.size();
}
